...
import org.irdresearch.smstarseel.autosys.DailySummaryNotifierJob;
...
public class testDsummary {
	private static void appendReturnedDataToCsvStrem(ByteArrayOutputStream fw, List dataSet, String mainHeading) throws IOException{
	 ...
	 try{
 		...
 		for (Project project : prjl) {
 			...
 			int setpid = ss.createSQLQuery("SET @projectId="+project.getProjectId()).executeUpdate();
  			List dsob = ss.createSQLQuery(DataQuery.QUICK_SUMMARY_OUTBOUND_QUERY).list();
 			...
 			//int setpid = ss.createSQLQuery("SET @projectId="+project.getProjectId()).executeUpdate();
  			List dscl = ss.createSQLQuery(DataQuery.QUICK_SUMMARY_CALLLOG_QUERY).list();
         ...
         //service.schedule(new DailySummaryNotifierJob(service),remainingTime,TimeUnit.MILLISECONDS);
